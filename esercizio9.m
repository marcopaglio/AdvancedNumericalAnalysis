% Utilizzando la function di esercizio8 verificare (in modo grafico) le
...
if num_nodes ~= length(nodes)
    error('Your nodes do not correspont to the number of nodes declareted!');
end
...
for j = 1 : num_nodes
    %call hermiteBasis
    [u_bases(j, 1: end), v_bases(j, 1: end)] = hermiteBasis(num_nodes, nodes, j, plotPoints);
    u_base_values_sum = u_base_values_sum + u_bases(j, 1: end);
    v_base_values_sum = v_base_values_sum + v_bases(j, 1: end);
    
    %draw U basis
    plot(plotPoints, u_bases(j, 1: end), '-.', 'DisplayName', strcat('U_{', int2str(j), '}(x)'));
    hold on;
end
...
title(strcat('U Hermite Basis for ', int2str(num_nodes), ' nodes'));
xlabel('x');
...
for j = 1 : num_nodes
    ...
    plot(plotPoints, v_bases(j, 1: end));
    hold on;
    ...
    for index_node = 1 : num_nodes
        ...
        plot(abscissas_tangent, ordinates_tangent, 'color', 'black','LineWidth', 1.5, 'HandleVisibility', 'off');
        hold on;
    end
